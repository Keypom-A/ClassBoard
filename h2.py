...
def init_db():
    ...
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            # 列の追加チェック（エラー回避用）
            cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name='tasks' AND column_name='is_notice'")
            if not cur.fetchone():
                cur.execute('ALTER TABLE tasks ADD COLUMN is_notice BOOLEAN DEFAULT FALSE')
            
            cur.execute("SELECT column_name FROM information_schema.columns WHERE table_name='chat_messages' AND column_name='file_path'")
            if not cur.fetchone():
                cur.execute('ALTER TABLE chat_messages ADD COLUMN file_path TEXT')
